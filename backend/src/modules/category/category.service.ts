...
export class CategoryService {
  ...
  async findByfields(
    options: FindOneOptions<CategoryDTO>
  ...
  async findAndCount(
    options: FindManyOptions<CategoryDTO>
  ): Promise<[CategoryDTO[], number]> {
    options.relations = relationshipNames;
    const resultList = await this.categoryRepository.findAndCount(options);
    const categoryDTO: CategoryDTO[] = [];
    if (resultList && resultList[0]) {
      resultList[0].forEach((category) =>
        categoryDTO.push(CategoryMapper.fromEntityToDTO(category))
      );
